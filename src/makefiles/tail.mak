...
OPTIONS_COMPILE:=$(CFLAGS) -DBUILD_DATE_Y=$(shell date +%Y) -DBUILD_DATE_M=$(shell date +%m | sed s/^0//) -DBUILD_DATE_D=$(shell date +%d | sed s/^0//) -DBUILD_DATE_HO=$(shell date +%H | sed s/^0//) -DBUILD_DATE_MI=$(shell date +%M  | sed s/^0//) -DBUILD_DATE_SE=$(shell date +%S | sed s/^0//) -DBUILD_PLACE='"$(shell hostname)"' -DBUILDER_NAME='"$(shell whoami)"' $(OPTIONS_COMPILE)
